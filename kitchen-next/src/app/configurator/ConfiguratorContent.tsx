...
export default function ConfiguratorContent({ product }) {
  useEffect(() => {
    track("configurator_open", { productId: product?.id });
  }, [product?.id]);
    ...
    <>
      ...
      <section className="flex flex-col" style={{ height: "calc(100dvh - 57px)" }}>
        ...
        <div className="min-h-0 flex-1">
          <KitchenConfigurator mode="fullscreen" product={product} />
        </div>
      </section>
    </>
  );
