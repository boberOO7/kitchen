...
export default function CartPageContent() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Step 1: Get orderId from server action
      ...
      // Step 2: Call API endpoint to create monobank invoice
      ...
      if (!response.ok) {
        setCheckoutError(data.error || "Не вдалося створити платіж");
        setIsCheckingOut(false);
        return;
      }

      // Step 3: Redirect to monobank payment page
      if (data.pageUrl) {
        window.location.href = data.pageUrl;
      } else {
        setCheckoutError("Не отримано посилання на оплату");
        setIsCheckingOut(false);
      }
    } catch (error) {
      console.error("Checkout error:", error);
      setCheckoutError("Сталася помилка. Спробуйте ще раз.");
      setIsCheckingOut(false);
    }
  };
                ...
                <AnimateOnScroll variant="fadeUp" delay={0.2}>
                  <div className="sticky top-24 rounded border border-[var(--sky-card-border)] bg-[var(--sky-card-bg)] p-6 shadow-[var(--sky-shadow)]">
                    ...
                    <button
                      ...
                      className="mt-6 w-full rounded bg-[var(--sky-accent)] px-4 py-4 text-sm font-medium text-[var(--sky-accent-fg)] transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isCheckingOut ? (
                    ...
                    {checkoutError && (
                      <div className="mt-4 rounded bg-red-50 p-3 text-center text-sm text-red-600">
                        {checkoutError}
                      </div>
                    )}
