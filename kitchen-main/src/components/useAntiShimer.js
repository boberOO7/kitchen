...
export function useAntiShimmer(root) {
  ...
  useEffect(() => {
    ...
    const setTex = (t) => {
      ...
      t.anisotropy = Math.min(maxAniso, 8);                      // 8 — sweet spot, можна maxAniso, або 16
      t.magFilter = THREE.LinearFilter;
      t.minFilter = THREE.LinearMipmapLinearFilter;              // трилінійна фільтрація або LinearMipmapNearestFilter
    ...
    root.traverse((o) => {
      ...
      mats.forEach((m) => {
        ...
        // зменшуємо спекулярне рябіння на «металі»
